// Social video scraper — Instagram Reels + TikTok.
// Appends every video (link + view count, when shown on the grid) into ONE
// combined CSV with a `platform` column. Run it for as many accounts / both
// platforms as you like; it all accumulates in the same file. Human-like jitter.
//
// Usage:
...
//   node scrape.mjs --tiktok grace.learns100                 # TikTok (no login needed)
//   node scrape.mjs --tiktok https://www.tiktok.com/@grace.learns100   # URLs ok too
//   node scrape.mjs --ig jess.studytips --tiktok grace.learns100       # mix in one run
//   node scrape.mjs --tiktok grace.learns100 --headless      # no visible window (tiktok only)
//
...
const targets = []; // { platform, type: 'account'|'post', value }
...
for (const a of argv) {
  ...
  else if (ttPost) t = { platform: "tiktok", type: "post", value: ttPost[1] };
  else {
...
const PLATFORMS = {
  ...
  tiktok: {
    ...
    // each post is [data-e2e="user-post-item"] with a /video/ link + view count
    extract: () => {
      const out = [];
      const posts = document.querySelectorAll('[data-e2e="user-post-item"]');
      const seen = new Set();
      const push = (a, viewsEl) => {
        const m = (a.getAttribute("href") || a.href || "").match(/\/video\/(\d+)/);
        if (!m || seen.has(m[1])) return;
        seen.add(m[1]);
        out.push({ id: m[1], url: `https://www.tiktok.com/@${location.pathname.split("/")[1].replace("@", "")}/video/${m[1]}`, text: (viewsEl?.innerText || "").trim() });
      };
      if (posts.length) {
        for (const item of posts) {
          const a = item.querySelector('a[href*="/video/"]');
          if (a) push(a, item.querySelector('[data-e2e="video-views"]'));
        }
      } else {
        for (const a of document.querySelectorAll('a[href*="/video/"]')) {
          const container = a.closest("div");
          push(a, container?.querySelector('[data-e2e="video-views"]') || container?.querySelector("strong"));
        }
      }
      return out;
    },
...
// ---- launch -----------------------------------------------------------------
const needIg = targets.some((t) => t.platform === "instagram") || LOGIN;
const haveStateFile = fs.existsSync(STATE);
const needLogin = needIg && (LOGIN || !haveStateFile);

const browser = await chromium.launch({ headless: HEADLESS && !needLogin });
const context = haveStateFile && !LOGIN
  ? await browser.newContext({ storageState: STATE, viewport: { width: 1280, height: 900 } })
  : await browser.newContext({ viewport: { width: 1280, height: 900 } });
const page = await context.newPage();

// ---- IG login (auto-detects success via sessionid cookie) -------------------
if (needLogin) {
  ...
  if (LOGIN && !targets.length) { await browser.close(); process.exit(0); }
}
...
for (let i = 0; i < targets.length; i++) {
  ...
  if (t.type === "post") {
    if (t.platform !== "instagram") { console.error(`  ! skipping non-IG post ${t.value} (TikTok set aside)`); continue; }
    try {
  ...
  if (i < targets.length - 1) await jitterSleep(8000, 20000); // polite gap
}
